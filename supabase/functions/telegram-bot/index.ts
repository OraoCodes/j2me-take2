...
// Track booking sessions by chat ID
...
async function getAIResponse(userMessage: string, context: { 
  ...
  if (bookingSession) {
    return await handleBookingStep(chatId, userMessage);
  }
  ...
  try {
    const prompt = `You are Wairimu, a helpful assistant for Kevin's beauty and wellness services. 
    You can answer questions about services, availability, and booking.
    Here is some information to help you:\n
    Available Services:\n${context.services.map(s => `- ${s.name}: KES${s.price}`).join('\n')}\n
    Availability:\n${context.availability.map(a => `- ${a.day_of_week}: ${a.start_time} - ${a.end_time}`).join('\n')}\n
    Recent Service Requests:\n${context.serviceRequests.map(r => `- ${r.customer_name} - ${r.services?.name} - ${r.scheduled_at}`).join('\n')}\n
    Blocked Dates:\n${context.blockedDates.map(d => `- ${d.blocked_date}`).join('\n')}\n
