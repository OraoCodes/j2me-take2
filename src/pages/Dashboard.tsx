...
const Dashboard = () => {
  ...
  const fetchCategories = async () => {
    const {
      data,
      error
    } = await supabase.from('service_categories').select('*').order('sequence', {
      ascending: true
    });
    if (error) {
      toast({
  ...
  const filteredCategories = categories.filter(category => activeTab === "visible" ? category.is_visible : !category.is_visible);
          ...
          <div className="max-w-5xl mx-auto">
            {showCategories && <>
                <div className="flex items-center justify-between mb-8">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <CreateCategoryDialog isOpen={isDialogOpen} onOpenChange={setIsDialogOpen} categoryName={newCategoryName} onCategoryNameChange={setNewCategoryName} onCreateCategory={createCategory} />
                  </div>
                </div>

                

                <EditCategoryDialog isOpen={!!selectedCategory} onClose={() => setSelectedCategory(null)} category={selectedCategory} onSave={handleEditCategory} onDelete={handleDeleteCategory} />
              </>}
            ...
            {showServices && <>
                <div className="flex items-center justify-between mb-8">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <Button onClick={() => navigate('/create-service')} className="bg-gradient-to-r from-gebeya-pink to-gebeya-orange text-white hover:opacity-90">
                ...
                <div className="bg-white rounded-lg shadow-sm border border-gray-100">
                  <div className="p-4 border-b">
                    <div className="flex items-center gap-4">
                      <div className="flex-1">
                        <Input type="text" placeholder="Search by service name" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="max-w-md focus:ring-gebeya-pink focus:border-gebeya-pink" />
                      </div>
                  ...
                  <div className="divide-y divide-gray-100">
                    {filteredServices.map(service => <div key={service.id} className="p-4 flex items-center hover:bg-gray-50/80">
                        ...
                        <div className="w-16 h-16 rounded bg-gray-100 mr-4">
                          {service.image_url && <img src={service.image_url} alt={service.name} className="w-full h-full object-cover rounded" />}
                        </div>
                        <div className="flex-1">
                        ...
                        <div className="flex items-center gap-4">
                          <Select value={service.category_id || ""} onValueChange={value => handleUpdateServiceCategory(service.id, value)}>
                          ...
                          <Button variant="ghost" size="sm" onClick={() => handleDeleteService(service.id)} className="hover:text-red-600 hover:bg-red-50">
                            <Trash2 className="w-4 h-4 text-red-500" />
                          </Button>
                          <span className={`px-2 py-1 rounded-full text-xs ${service.is_active ? 'bg-gebeya-pink/10 text-gebeya-pink' : 'bg-gray-100 text-gray-800'}`}>
            ...
            {!showCategories && !showServices && <>
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 mb-8">
                  <div className="p-6">
                    ...
                    <div className="space-y-4">
                      {setupSteps.map(step => <div key={step.number} className="flex items-center gap-4">
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center ${step.completed ? "bg-green-100 text-green-600" : "bg-gray-100 text-gray-600"}`}>
                            {step.completed ? <svg className="w-5 h-5" viewBox="0 0 20 20" fill="currentColor">
                          ...
                          <Button variant={step.action === "Upgrade" ? "default" : "outline"} className={step.action === "Upgrade" ? "bg-blue-500 hover:bg-blue-600" : ""} onClick={step.onClick}>
                            {step.action}
                          </Button>
                        </div>)}
                    </div>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                  ...
                  label: "Helpdesk"
                }].map(item => <a key={item.label} href="#" className="flex items-center justify-between p-4 rounded-lg border border-gray-100 hover:bg-gray-50">
                        <div className="flex items-center gap-3">
                          {item.icon}
